...
const QDBusArgument &
operator>>(const QDBusArgument &arg, QXdgDesktopPortalFileDialog::FilterCondition &filterCondition)
{
    uint type;
...
QDBusArgument &
operator<<(QDBusArgument &arg, const QXdgDesktopPortalFileDialog::Filter filter)
{
    arg.beginStructure();
...
const QDBusArgument &
operator>>(const QDBusArgument &arg, QXdgDesktopPortalFileDialog::Filter &filter)
{
    QString name;
...
void
QXdgDesktopPortalFileDialog::initializeDialog()
{
    Q_D(QXdgDesktopPortalFileDialog);
...
void
QXdgDesktopPortalFileDialog::openPortal(Qt::WindowFlags windowFlags,
                                        ...
                                        QWindow *parent)
{
    ...
    QDBusMessage message = QDBusMessage::createMethodCall("org.freedesktop.portal.Desktop",
                                                          ...
                                                          d->saveFile ? "SaveFile" : "OpenFile");
    QVariantMap options;
    if (!d->acceptLabel.isEmpty())
        options.insert("accept_label", d->acceptLabel);

    options.insert("modal", windowModality != Qt::NonModal);
    options.insert("multiple", d->multipleFiles);
    options.insert("directory", d->directoryMode);

    if (!d->directory.isEmpty())
        options.insert("current_folder", QFile::encodeName(d->directory).append('\0'));
    ...
    if (d->saveFile && !d->selectedFiles.isEmpty()) {
        ...
        if (selectedFileInfo.exists())
            options.insert("current_file",
                           QFile::encodeName(d->selectedFiles.constFirst()).append('\0'));
        options.insert("current_name", selectedFileInfo.fileName());
    }
    ...
    if (!filterList.isEmpty())
        options.insert("filters", QVariant::fromValue(filterList));

    if (selectedFilterIndex != -1)
        options.insert("current_filter", QVariant::fromValue(filterList[selectedFilterIndex]));
...
bool
QXdgDesktopPortalFileDialog::defaultNameFilterDisables() const
{
    return false;
}

void
QXdgDesktopPortalFileDialog::setDirectory(const QUrl &directory)
{
    Q_D(QXdgDesktopPortalFileDialog);
...
QUrl
QXdgDesktopPortalFileDialog::directory() const
{
...
void
QXdgDesktopPortalFileDialog::selectFile(const QUrl &filename)
{
    Q_D(QXdgDesktopPortalFileDialog);
...
QList<QUrl>
QXdgDesktopPortalFileDialog::selectedFiles() const
{
    Q_D(const QXdgDesktopPortalFileDialog);
...
void
QXdgDesktopPortalFileDialog::selectMimeTypeFilter(const QString &filter)
{
    Q_D(QXdgDesktopPortalFileDialog);
    if (d->nativeFileDialog) {
        d->nativeFileDialog->setOptions(options());
...
QString
QXdgDesktopPortalFileDialog::selectedMimeTypeFilter() const
{
    Q_D(const QXdgDesktopPortalFileDialog);
...
void
QXdgDesktopPortalFileDialog::selectNameFilter(const QString &filter)
{
    Q_D(QXdgDesktopPortalFileDialog);
...
QString
QXdgDesktopPortalFileDialog::selectedNameFilter() const
{
    Q_D(const QXdgDesktopPortalFileDialog);
...
bool
QXdgDesktopPortalFileDialog::show(Qt::WindowFlags windowFlags,
                                  Qt::WindowModality windowModality,
                                  QWindow *parent)
{
    Q_D(QXdgDesktopPortalFileDialog);
...
void
QXdgDesktopPortalFileDialog::gotResponse(uint response, const QVariantMap &results)
{
    Q_D(QXdgDesktopPortalFileDialog);

    if (!response) {
        if (results.contains("uris"))
            d->selectedFiles = results.value("uris").toStringList();

        if (results.contains("current_filter")) {
...
QXdgDesktopPortalFileDialog::useNativeFileDialog(
  QXdgDesktopPortalFileDialog::FallbackType fallbackType) const
{
    Q_D(const QXdgDesktopPortalFileDialog);
